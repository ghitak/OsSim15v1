...
public class Functions {
	...
	public void openMCQSimulationBD(QR qr) throws SoSimException{
		
//		if ( qr.getModuleQR()!=Constants.MODULE_MEMOIRE || qr.getModuleQR()!=Constants.MODULE_PROCESS	)
//			throw new SoSimException("all_04");
		...
		AppSession.getInstance().getPresenter().loadBD(qr);	// Load file
		AppSession.getInstance().getPresenter().updateInfo(); // Update table info
		AppSession.getInstance().getPresenter().repaintPainters(); // Repaint painters 
	}
